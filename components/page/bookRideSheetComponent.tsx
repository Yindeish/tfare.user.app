  ...
  return (
    <PaddedScreen>
      <View style={[flexCol, gap(16), mt(20)]}>
        ...
        // Here's a fixed version of your FlatList component:
        <FlatList
          style={[
            h(46),
            flex, // Make sure this doesn't restrict width
            gap(16),
          ...
          contentContainerStyle={[
            tw`w-auto`,
            // Don't restrict the content width for horizontal lists
            { paddingRight: 20 }, // Add some padding at the end
          ]}
          horizontal
          showsHorizontalScrollIndicator={true} // Make scrollbar visible for debugging
          ...
          renderItem={({ item: busstop }) => (
            <TouchableOpacity onPress={() => selectPickup(busstop)}>
              <View
                style={[
                  ...
                  justifyCenter,
                  {
                    borderWidth: 1,
                    borderColor:
                      values?.pickupBusstop == busstop?.name
              ...
              >
                <Image
                  style={tw`w-[22px] h-[19px]`}
                  source={
                    values?.pickupBusstop == busstop?.name
                      ? images.whiteBgTripImage
                      : images.trip
                  }
                />
              ...
              <View
                style={[
                  ...
                  justifyCenter,
                  {
                    borderWidth: 1,
                    borderColor:
                      values?.dropoffBusstop == busstop?.name
                        ? Colors.light.background
                        : Colors.light.border,
                    backgroundColor:
                      values?.dropoffBusstop == busstop?.name
...
function TicketDetailsSheetWithNoSelection() {
  const dispatch = useAppDispatch();
  const [[_, query], setQuery] = useStorageState(RideConstants.localDB.query);
  const { token: session } = useAppSelector((state: RootState) => state.user);
  const { savedAddresses } = AccountSelectors();
