  ...
  const getHistory = async () => {
    ...
    if (sameTickets) {
      const tickets = rideHistory
        ?.filter(
          (rideHist: IRiderRideDetails) => rideHist?.rideStatus === "booked"
        )
        ?.map((rideHist: IRiderRideDetails, index: number) =>
          createTicket(rideHist, paidTicket?.ticketOtp, index)
        );
      ...
      const tickets = paidTicket
        ?.filter((ticketItem) => {
          const riderRide = (rideHistory as IRiderRideDetails[])?.find(
            (ride) =>
              String(ride?._id) ==
                String(ticketItem?.ride?.riderRideDetailsId) &&
              ride?.rideStatus == "booked"
          );
          return riderRide;
        })
          ...
          const riderRide = (rideHistory as IRiderRideDetails[])?.find(
            (ride) =>
              String(ride?._id) ==
                String(ticketItem?.ride?.riderRideDetailsId) &&
              ride?.rideStatus == "booked"
          ) as IRiderRideDetails;

          const newTicket = createTicket(
            riderRide,
            ticketItem?.ticketOtp,
            index
          );
              ...
              <View key={index}>
                <TouchableOpacity
                  onPress={() => {
                    ...
                    if(item?.rideStatus === 'ended') {
                      dispatch(setState({key: 'activeTab', value: 'completed'}))
                      router.push('/(rideScreens)/tripHistory')
                    }
                  }}
                  style={[
