...
const initialState: IRideState = {
  ...
  sameTickets: [],
  lastRides: [],
  currentRideUnderNegotiation: null,
  currentRoute: null,
  tripId: "",
  stateInput: {
...
const RideSlice = createSlice({
  ...
  reducers: {
    setState: (
      state,
      action: PayloadAction<{ key: keyof IRideState; value: any }>
    ) => {
    ...
    setStateInputField: (
      state,
      action: PayloadAction<{ key: keyof IStateInput; value: any }>
    ) => {
      const { key, value } = action.payload;
    ...
    createTicket: (
      state,
      action: PayloadAction<{ currentNumberOfTickets: number }>
    ) => {
      ...
      for (let val = 0; val < currentNumberOfTickets; val++) {
        const newTicket: ITicketInput = {
          dropoffBusstop:
            val + 1 === 1
              ? state.stateInput.dropoffBusstopInput ||
                state.riderRideDetails?.dropoffBusstop
              : null, // setting the first ticket to use the underlying credentials
          pickupBusstop:
            val + 1 === 1
              ? state.stateInput.pickupBusstopInput ||
                state.riderRideDetails?.pickupBusstop
              : null, // setting the first ticket to use the underlying credentials
          owner: {},
          ticketOtp: "",
          sameAsFirstTicket: val + 1 === 1 ? true : false, // setting the first ticket to use the underlying credentials
          number: val + 1,
          userCounterFare:
            val + 1 === 1 ? state?.riderRideDetails?.riderCounterOffer : null,
          // rideFee: (val + 1) === 1 ? Number(state?.riderRideDetails?.ridePlan?.plan?.ride?.rideFee) : null as never,
          rideFee:
            val + 1 === 1
              ? Number(
                  state?.riderRideDetails?.riderCounterOffer ||
    ...
    toggleTicketAsFirstTicket: (
      ...
      if (state.stateInput.userRideInput.tickets) {
        ...
        const firstTicket = state.stateInput.userRideInput.tickets.find(
          (ticket) => Number(ticket.number) === 1
        );
        console.log({ ___firstTicket___: firstTicket });

        const ticket = state.stateInput.userRideInput.tickets.find(
          (ticket) => Number(ticket.number) === Number(currentNumberOfTickets)
        );

        if (ticket) {
          state.stateInput.userRideInput.tickets =
            state.stateInput.userRideInput.tickets.map((ticket) => {
    ...
    editTicketBusstops: (
      ...
      const { currentNumberOfTickets, ridePlan } = action.payload;
          ...
          state.stateInput.userRideInput.tickets?.map((ticket) => {
            if (Number(ticket.number) === Number(currentNumberOfTickets)) {
              return {
                ...
                sameAsFirstTicket: false,
                rideFee: ridePlan ? ridePlan?.ride?.rideFee || ridePlan?.plan?.ride?.rideFee : null,
              };
            } else return ticket;
          }) as ITicketInput[];
      } else return;
    ...
    setCounterFareStatus: (
      state,
      action: PayloadAction<TCounterFareStatus>
    ) => {
