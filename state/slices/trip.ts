...
const RideSlice = createSlice({
    ...
    reducers: {
        ...
        setUserRide: (state, action: PayloadAction<{riderRideDetails: IRiderRideDetails, currentTrip: ICurrentTrip}>) => {
            ...
            // if (state.stateInput.userRideInput && state.stateInput.userRideInput?.tickets) {
                ...
                for (let val = 0; val < (currentNumberOfTickets); val++) {
                    const newTicket: ITicketInput = {
                        ...
                        rideFee: (val + 1) === 1 ? Number(state?.riderRideDetails?.riderCounterOffer || state?.riderRideDetails?.ridePlan?.ride?.rideFee) : null as never,
                    }
